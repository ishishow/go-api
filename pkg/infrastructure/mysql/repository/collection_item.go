...
func NewCollectionItemRepositoryImpl(sqlHandler mysql.SQLHandler) db.CollectionItemRepository {
...
func (uri collectionItemRepositoryImpl) SelectAllCollectionItems() ([]*cm.CollectionItem, error) {
	rows, err := uri.SQLHandler.Conn.Query("SELECT * FROM collection_item")
	if err != nil {
		return nil, err
	}
...
func (uri collectionItemRepositoryImpl) SelectCollectionItemsByPrimaryKeys(collectionItemIDs []string) ([]*cm.CollectionItem, error) {
	...
	sqlRaw := fmt.Sprintf(`SELECT * FROM collection_item WHERE id IN ('%s')`, ids)
