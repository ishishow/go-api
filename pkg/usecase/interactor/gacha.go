...
func (uu gachaUseCase) GachaDraw(ctx context.Context, userID string, times int32) ([]*GachaResult, error) {
	user, err := uu.userRepository.SelectByAuthToken(userID) //TODO:
	if err != nil {
		return nil, err
	}
	...
	userCollectionItems, err := uu.userCollectionItemRepository.SelectUserCollectionItemsByUserID(user.ID)
	if err != nil {
		return nil, err
	}
